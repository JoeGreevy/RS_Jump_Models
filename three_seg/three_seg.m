...
%%% 03/04/26 Adding scale and angles to perform orientation registration.
%%%%%%%%%%%%%%%
...
markers = [];
load("marker_list.mat", "marker_list");
...
%%% Method used in orientation, registration where markers are already on
%%% the model
% c3dName = "R_InStep"; osimName = "instep"; osimBod = "foot";
% point_in_local = ground.findStationLocationInAnotherFrame(state, osimVec3FromArray(mRegLocs.(c3dName)), mbs.get(osimBod));
% mms.get(osimName).set_location(point_in_local);
model.print("three_seg_reg.osim")
