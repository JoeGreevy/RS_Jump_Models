%%%%%%%%%%%%%%%%%%%%%
%%%% 24/03/26
%%%% Constructing a single segment model which I can hopefully push
%%%% experimental data onto
%%%%%%%%%%%%%%%%%%%%%
...
thigh.addComponent(thigh_offset); thigh_offset.attachGeometry(cyl.clone());
...
if exist(model.getName().toCharArray' + "_man_scale.osim", "file")
    ...
    markers = [markers, Marker("gt", thigh, Vec3(0, 0.45, 0.2));
        Marker("lfe", thigh, Vec3(0.01, -0.45, 0.1))]; %, ...
        %%Marker("mfe", thigh, Vec3(0.01, -0.44, -0.2))];
end
...
model.print(string(model.getName()) + ".osim")